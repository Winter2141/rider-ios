...
class InvoiceView: UIView {
    ...
    var onRazorPay : ((String)->Void)?
    var onClickPaynow : ((String)->Void)?
...
//        }
        ...
        PaymentType = request.payment_mode ?? ""
        if request.payment_mode == "RAZORPAY" || request.payment_mode == "PAYPAL"{
...
//        lblName.text = request.user?.fri
        self.lblName.text = "\(request.user?.first_name ?? "") Owns"
        self.lblPrice.text = "\(String.removeNil(User.main.currency)) \(request.payment?.total ?? "")"
    }
...
//        self.onClickPaynow?(self.tipsAmount)
        ...
        if PaymentType == "RAZORPAY" || PaymentType == "PAYPAL" {
            
            let amount : Float = Float(request?.payment?.total ?? "") ?? 0
            
            self.onRazorPay?("\(amount * 100)")
//            self.onRazorPay?("111" , "100")
    ...
    func payment_id(id : String)  {
        self.onClickPaynow?(id)
    }
    //  Change Payment Type
