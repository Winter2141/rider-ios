...
internal func createActivityIndicator(_ uiView : UIView)->UIView{
    ...
    loadingView.backgroundColor = hexStringToUIColor(hex: Constants.BLACK_COLOUR)
...
internal func retrieveUserData()->Bool{
    ...
    if (UserDefaults.standard.object(forKey: Keys.list.userData) as? Data) == nil{
       return false
    }
    else
    {
...
func forceLogout(with message : String? = nil) {
    ...
    let navigationController = UINavigationController(rootViewController: Router.user.instantiateViewController(withIdentifier: Storyboard.Ids.EmailViewController))
