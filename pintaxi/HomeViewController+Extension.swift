...
//        self.isOnBooking = true
        if self.estimationFareView == nil {
            ...
            _ = 0 //Float(User.main.wallet_balance?? "") != 0
            let heightPadding : CGFloat = 0 //(isWalletAvailable ? 0 : 40)
    ...
    func removeStatusView(){
        self.statusTypeView?.dismissView(onCompletion: {
            self.statusTypeView = nil
        })
    ...
    func showRideStatusView(with request : Request) {
        ...
        if self.rideStatusView == nil, let rideStatus =
            Bundle.main.loadNibNamed(XIB.Names.RideStatusView, owner: self, options: [:])?.first as? RideStatusView {
            rideStatus.frame = CGRect(origin: CGPoint(x: 0, y: self.view.frame.height-380), size: CGSize(width: self.view.frame.width, height: 380))
...
//            self.addFloatingButton(with: rideStatus.frame.height, to: request.provider)
        ...
        if let latitude = request.provider?.latitude, let longitude = request.provider?.longitude {
            ...
            self.moveProviderMarker(to: LocationCoordinate(latitude: Double(latitude)!, longitude: Double(longitude)!), bearing: 12.0)
        }
        ...
        rideStatusView?.onChatView = { id , requestId in
            self.ChatView(orderId: "\(id)" ,requestId: "\(requestId)")
        }
        
        rideStatusView?.onClickPickupNote = {
            let chatVC = self.storyboard?.instantiateViewController(withIdentifier: "PickupNoteViewController") as! PickupNoteViewController
    ...
    func ChatView(orderId : String, requestId : String){
        let chatVC = storyboard?.instantiateViewController(withIdentifier: "ChatVC") as! ChatVC
        chatVC.requestID = requestId
        chatVC.providerID = orderId
    ...
    func showInvoiceView(with request : Request) {
        ...
        if self.invoiceView == nil, let invoice = Bundle.main.loadNibNamed(XIB.Names.InvoiceView, owner: self, options: [:])?.first as? InvoiceView {
            ...
            self.invoiceView?.onClickPaynow = { tipsAmount in
                ...
                requestObj.total_payment = request.payment?.total
                requestObj.payment_id = tipsAmount
//                if tipsAmount>0 {
...
//                }
            ...
            self.invoiceView?.onRazorPay = { amount in
                self.showPaymentForm(amount: amount, orderId: "")
            }
...
//            }
    ...
    func showRatingView(with request : Request) {
        ...
        if let rating = Bundle.main.loadNibNamed(XIB.Names.RatingView, owner: self, options: [:])?.first as? RatingView {
            ...
            self.viewAddressOuter.isHidden = true
            self.viewLocationButtons.isHidden = true
            rating.frame = CGRect(origin: CGPoint(x: 0, y: self.view.frame.height - 500), size: CGSize(width: self.view.frame.width, height: 500))
            ratingView = rating
    ...
    func showProviderInCurrentLocation(with data : [Provider],serviceTypeID:Int) {
        ...
        for locationData in data where locationData.longitude != nil && locationData.latitude != nil {
            ...
            if serviceTypeID == 0 {
                ...
                let lati = Double(locationData.latitude ?? "")
                let long = Double(locationData.longitude ?? "")
                
                marker.position = CLLocationCoordinate2DMake(lati ?? 0.0, long ?? 0.0)
        ...
        if self.requestLoaderView == nil, let singleView = Bundle.main.loadNibNamed(XIB.Names.LoaderView, owner: self, options: [:])?.first as? LoaderView {
          //  self.isOnBooking = true
            ...
            self.requestLoaderView?.onCancel = {
                self.cancelCurrentRide(isSendReason: false)
            }
            self.view.addSubview(singleView)
            DispatchQueue.main.asyncAfter(deadline: .now()) { // Hiding Address View
...
//        self.requestLoaderView?.isCancelButtonEnabled = requestId != nil
    ...
    private func showCancelReasonView(completion : @escaping ((String)->Void)) {
        ...
        if self.reasonView == nil, let reasonView = Bundle.main.loadNibNamed(XIB.Names.ReasonView, owner: self, options: [:])?.first as? ReasonView {
            reasonView.frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height)
...
//        }
        if [RideStatus.searching,RideStatus.started,RideStatus.arrived,RideStatus.completed].contains(status) {
            ...
            self.viewAddressOuter.isHidden = true
             ...
             collectionService.isHidden = true
        }else{
        ...
        case .accepted, .arrived, .started, .pickedup:
            con_mapView.constant = -260

            self.showRideStatusView(with: request)
            if status == .pickedup {
                self.showRequestActivity(strType:"PICKEDUP")
            }
            if status == .arrived{
                self.showRequestActivity(strType:"ARRIVED")
            }
            if status == .started{
                self.showRequestActivity(strType:"STARTED")
            }
...
//            self.viewEstimated.isHidden = true
        ...
        case .dropped:
            self.viewAddressOuter.isHidden = true
            self.showRequestActivity(strType:"DROPPED")
...
//            }
    ...
    func showRequestActivity(strType:String){
        
       
        
        
//        if self.statusTypeView == nil, let status = Bundle.main.loadNibNamed(XIB.Names.status, owner: self, options: [:])?.first as? StatusTypeView {
//            status.frame = CGRect(origin: CGPoint(x: 0, y:50), size: CGSize(width: self.view.frame.width, height: status.frame.height))
//            self.statusTypeView = status
//            self.statusTypeView?.clipsToBounds = false
//            self.statusTypeView?.show(with: .bottom, completion: nil)
//            self.view.addSubview(self.statusTypeView!)
//            self.statusTypeView?.statusType(strType:strType)
//
//        }else{
//            self.statusTypeView?.statusType(strType:strType)
//
//        }
    ...
    private func cancelCurrentRide(isSendReason : Bool = false) {
        ...
        let alert = UIAlertController(title: "iLtft", message: Constants.string.cancelRequestDescription.localize(), preferredStyle: UIAlertController.Style.alert)
...
//        UIApplication.topViewController()?.present(alert, animated: true, completion: nil)
        ...
        func cancelRide(reason : String? = nil) { // Cancel Ride
            ...
            removeUnnecessaryView(with: .cancelled)
            clearMapview()
            removeRideNow()
            removeStatusView()
    ...
    private func addFloatingButton(with padding : CGFloat, to provider : Provider?) {
        ...
        if provider != nil {
            ...
            floaty.addItem(icon: #imageLiteral(resourceName: "chatIcon").resizeImage(newWidth: 25)) { (_) in
                if let vc = self.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.ChatVC) as? ChatVC {
                    let navigation = UINavigationController(rootViewController: vc)
                    self.present(navigation, animated: true, completion: nil)
                }
            }
    ...
    func showETA(with providerLocation : LocationCoordinate) {
        ...
        self.mapViewHelper?.mapView?.getEstimation(between: source, to: destination, completion: { (estimation) in
            DispatchQueue.main.async {
                ...
                self.setTimeStatus = estimation
//                self.mapViewHelper?.mapView?.reloadInputViews()
